...
assert(app.includes("useEffect(() => {\n  if (!operatorSession || !canReadExecutions) return\n\n  const stored ="), 'restauração operacional exige autenticação e capability de execução')
assert(app.includes("}, [configurationRevision, loadActionDetail, operatorSession?.token, canReadExecutions])"), 'restauração não reage à sessão autenticada e às capabilities')
