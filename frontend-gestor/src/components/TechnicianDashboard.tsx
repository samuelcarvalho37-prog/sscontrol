...
import type { Execution, ExecutionCompletionBlocker, ExecutionItem, OperatorAction, OperatorActionDetail, StopMode, TechnicalCompletionInput } from '../types/operatorActions'
...
function planText(value: AdminEntityRecord, key: string) {
  return String(value[key] ?? '').trim()
}

function normalized(value: string) {
  return value.normalize('NFD').replace(/[\u0300-\u036f]/gu, '').toUpperCase()
}
...
function TechnicianDashboardContent({ onSessionExpired }: { onSessionExpired: () => void }) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const [operatorActions, technicianPlans] = await Promise.all([listOperatorActions(), listTechnicianPlans()])
  ...
  const visiblePlans = useMemo(() => plans.filter((plan) => {
    const versionStatus = normalized(planText(plan, 'versao_status') || planText(plan, 'workflow_status'))
    const lifecycle = normalized(planText(plan, 'status'))
    return versionStatus === 'PUBLISHED' && ['ACTIVE', 'ATIVO', ''].includes(lifecycle)
  }), [plans])

  async function open(actionId: string) {
    setBusy(true); setError(''); setBlockers([])
    try { const data = await getOperatorAction(actionId); setSelected(data.acao); setExecution(data.execucao); setMode(data.execucao?.modo_parada ?? data.acao.modo_parada ?? 'NO_STOP'); setCompletion(current => ({ ...current, modo_parada: data.execucao?.modo_parada ?? data.acao.modo_parada ?? 'NO_STOP' })) }
  ...
  return <section className="technician-dashboard" aria-busy={loading || busy}>
    <header className="technician-dashboard__heading"><div><span className="eyebrow">EXECUÇÃO DE MANUTENÇÃO</span><h1>Central do Técnico</h1><p>Suas ordens executáveis e o checklist operacional em um só lugar.</p></div><button type="button" onClick={() => void reload()} disabled={loading || busy}>Atualizar</button></header>
    {error && <p className="technician-dashboard__error" role="alert">{error}</p>}
    <div className="technician-dashboard__metrics"><Metric label="Pendentes" value={counts.pending} /><Metric label="Em execução" value={counts.progress} /><Metric label="Críticas" value={counts.critical} tone="danger" /><Metric label="Atrasadas" value={counts.overdue} tone="warning" /></div>
    {!selected ? <>
      <section className="technician-dashboard__panel"><div className="technician-dashboard__panel-heading"><div><span className="pcm-section-kicker">MINHAS ORDENS</span><h2>Minhas execuções</h2></div><span>{actions.length} disponível(is)</span></div>{loading ? <p>Carregando suas ações…</p> : actions.length === 0 ? <p>Nenhuma ação executável foi encontrada para você.</p> : <div className="technician-dashboard__list">{actions.map(action => { const schedule = scheduleIndicator(action.programada_para, null, action.status); return <button type="button" className="technician-action" onClick={() => void open(action.id)} key={action.id}><span className="technician-action__top"><strong>{action.ordem_codigo}</strong><em>{label(action.prioridade)}</em></span><strong>{action.titulo}</strong><span>{[action.ativo_tag, action.ativo_nome].filter(Boolean).join(' · ')}</span><small>{[action.setor_nome, action.linha_nome, label(action.tipo)].filter(Boolean).join(' · ')} · {date(action.programada_para)}</small><span className={`technician-schedule-indicator technician-schedule-indicator--${schedule.tone}`}>{schedule.label}</span></button> })}</div>}</section>
      <section className="technician-dashboard__panel technician-dashboard__plans" aria-labelledby="technician-plans-title">
        <div className="technician-dashboard__panel-heading"><div><span className="pcm-section-kicker">PLANEJAMENTO</span><h2 id="technician-plans-title">Planos preventivos</h2></div><span>{visiblePlans.length} disponível(is)</span></div>
        <p>Consulte o planejamento ativo. Para executar, o PCM deve programar, liberar e atribuir a OS à sua equipe.</p>
        {loading ? <p>Carregando planos…</p> : visiblePlans.length === 0 ? <p>Nenhum plano preventivo publicado está disponível para consulta.</p> : <div className="technician-dashboard__plans-list">{visiblePlans.slice(0, 12).map(plan => <article key={`${plan.id}-${plan.versao_id ?? ''}`}><strong>{planText(plan, 'codigo') || 'Plano sem código'}</strong><span>{planText(plan, 'nome') || 'Plano sem nome'}</span><small>{[planText(plan, 'ativo_tag'), planText(plan, 'ativo_nome'), label(planText(plan, 'tipo'))].filter(Boolean).join(' · ') || 'Ativo não informado'}{planText(plan, 'recorrencia_dias') ? ` · a cada ${planText(plan, 'recorrencia_dias')} dias` : ' · agendamento manual'}</small></article>)}</div>}
        {visiblePlans.length > 12 ? <small className="technician-dashboard__plans-more">Exibindo 12 de {visiblePlans.length} planos publicados.</small> : null}
      </section>
...
function Metric({ label, value, tone = '' }: { label: string; value: number; tone?: string }) { return <article className={`technician-dashboard__metric ${tone}`}><span>{label}</span><strong>{value}</strong></article> }
